...
* **`<sourcesystem>_<entity>`**
  ...
  * Examples:
    ...
    * `crm_sales_details` → Sales transaction details from the CRM system.
    * `erp_cust_az12` → Customer-related information from the ERP system.
...
* **`dwh_<column_name>`**
  ...
  * Examples:
    ...
    * `dwh_insert_date` → Date when the record was inserted.
